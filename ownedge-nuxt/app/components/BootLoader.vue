...
const bootPointerEvents = computed(() => props.isBooted ? 'none' : 'auto');
const bootVideoOpacity = computed(() => props.isBooted ? 0 : 0.26);
const bootVideoFilter = computed(() => props.isBooted ? 'brightness(0.5)' : 'brightness(1)');
    ...
    return new Promise(resolve => {
        setTimeout(() => {
            try {
                ...
                // Try sound, ignore errors to prevent hanging
                try { SoundManager.playTypingSound(); } catch (e) { console.error(e); }
            } catch (err) {
                console.error("BootLoader addMessage error:", err);
            }
            resolve();
        }, delay);
    });
...
const runBiosSequence = async () => {
  try {
    console.log("BootLoader: Starting BIOS Sequence");
    emit('progress', 0);
    ...
    try { SoundManager.playTypingSound(); } catch (e) {}
    emit('progress', 20);
    ...
    console.log("BootLoader: Sequence Complete, showing login");
    bootStage.value = 'login';
    await nextTick();
    if (nicknameInputRef.value) nicknameInputRef.value.focus();
    
 } catch (err) {
     console.error("BootLoader Critical Error:", err);
     // Fallback to allow entry if it crashes
     bootStage.value = 'login';
 }
};
...
.boot-loader {
  ...
  width: 100%;
  height: 100%;
  z-index: 30; /* Above page content (5) and background (10) */
  overflow: hidden;
  /* Disable pointer events after boot to avoid blocking content */
  pointer-events: v-bind(bootPointerEvents);
}
...
.boot-video {
    ...
    height: 101%;
    object-fit: cover;
    opacity: v-bind(bootVideoOpacity); 
    mix-blend-mode: color-burn;
    filter: grayscale(0.6) contrast(2.3) v-bind(bootVideoFilter);
